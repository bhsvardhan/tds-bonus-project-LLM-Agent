class GyaanSetu {
          ...
          input.addEventListener("input", () => {
            ...
            if (valueSpan)
              valueSpan.textContent =
                id === "speech-rate" ? `${input.value}x` : input.value;
          });
      });
  ...
  displayMessage(message) {
    try {
      ...
      const senderName =
        { user: "You", assistant: "GyaanSetu", system: "System", tool: "Tool" }[
          message.role
        ] || message.role;
      const avatarIcon =
        {
  ...
  populateSettingsForm() {
    ...
    try {
      document.querySelector(
        `input[name="theme"][value="${s.ui.theme}"]`
      ).checked = true;
    } catch (e) {}
    document.getElementById("animations-enabled").checked =
      !!s.ui.animationsEnabled;
    ...
    document.getElementById("voice-output-enabled").checked =
      !!s.voice.outputEnabled;
    document.getElementById("voice-language").value =
      s.voice.language || "en-US";
    ...
    document.getElementById("analytics-enabled").checked =
      !!s.advanced.analyticsEnabled;
  ...
  updateSettingsFromForm() {
    ...
    s.ui.fontSize =
      document.getElementById("font-size")?.value || s.ui.fontSize;
    s.voice.enabled = document.getElementById("voice-enabled")?.checked;
    s.voice.outputEnabled = document.getElementById(
      "voice-output-enabled"
    )?.checked;
    s.voice.language =
      document.getElementById("voice-language")?.value || s.voice.language;
    s.voice.speechRate = parseFloat(
      document.getElementById("speech-rate")?.value || s.voice.speechRate
    );
    s.advanced.autoSave = document.getElementById("auto-save")?.checked;
    s.advanced.analyticsEnabled =
      document.getElementById("analytics-enabled")?.checked;
    s.advanced.maxHistory = parseInt(
      document.getElementById("max-history")?.value || s.advanced.maxHistory,
      10
    );
  ...
  applySettings() {
    try {
      this.updateTheme();
      const font =
        { small: "0.875rem", medium: "1rem", large: "1.125rem" }[
          this.state.settings.ui.fontSize
        ] || "1rem";
  ...
  showWelcomeMessage() {
    ...
    if (conv && conv.messages.length === 0) {
      this.addMessage(
        "assistant",
        "Welcome to GyaanSetu! How can I assist you?"
      );
    }
  }
  ...
  handleFiles(files) {
    files.forEach((file) => {
      if (
        ...
        this.addMessage(
          "system",
          `File uploaded: ${file.name} (size: ${file.size} bytes). Processing placeholder added.`,
          convId
        );
  ...
  exportConversation() {
    try {
      ...
      conv.messages.forEach((msg) => {
        const sender =
          (msg.role || "unknown").charAt(0).toUpperCase() +
      ...
      this.showToast(
        "error",
        "Export Failed",
        e.message || "Could not export conversation"
      );
    }
  }
